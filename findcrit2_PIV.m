function [x, McSmooth, startpt] = findcrit2_PIV(inputdata, opt, smoothingParam)
...
while ii < xsize
    ...
    % cutting out all irrelevant data (e.g. zeros around mask)
    rel_wData_start	= find(workingData ~= 0,1,'first');
    rel_wData_end   = find(workingData ~= 0,1,'last');
    
    % using only what is relevant to the measurement
    workingData     = workingData(rel_wData_start:rel_wData_end);

    % find transition between differnet zones in y-direction
    if strcmpi(opt, 'TI')
            transpos = find(workingData < max(workingData)/2, 2,'first');
            transpos = transpos(end);
    else
        transpos = find(workingData > mean(workingData)-std(workingData),1,'first');
    end
    
    if floor(transpos) < rel_wData_start
        transpos = 0;
    end
   
    transfind(positionImg) = transpos;

    ii          = ii + stepsize;
end

x               = 1:xsize;
McSmooth        = fit(x',transfind','smoothingspline',...
                        'SmoothingParam', smoothingParam);
[~, d_rive2]    = differentiate(McSmooth, x);

d_rive2         = fit(x',d_rive2,'smoothingspline',...
                        'SmoothingParam', smoothingParam);
% d_rive3         = differentiate(d_rive2, x);

startpt         = find(islocalmax(d_rive2(x),'MinProminence',0.002) == 1,1,'first');              % old: find(McSmooth(x) >= 2.5,1,'first');

if isempty(startpt)
    startpt     = 0;
end

if McSmooth(startpt) > 3
    startpt = find(McSmooth(x) > 3, 1, 'first');
end

% startpt         = find(d_rive2(x) == max(d_rive2(x)), 1, 'first');
% if isempty(startpt)
%     startpt     = 0;
% end
...
% startpt     = find(std(test_OG) == 0,1,'last')+1;
